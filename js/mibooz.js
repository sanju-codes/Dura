(function ($) {
  ...
  window.batchDOMUpdates = function(reads, writes) {
    // Perform all reads first
    const readResults = reads ? reads() : undefined;
    // Schedule writes in next frame
    if (writes) {
      requestAnimationFrame(() => writes(readResults));
    }
  };
  ...
  //     return document.querySelector('.main-slider-two__img')?.offsetWidth;
  //   },
  //   (width) => {
  //     // Write style changes here
  //     if (width) document.querySelector('.main-slider-two__img').style.width = width + 'px';
  //   }
  // );
  ...
  const validateStep = (step) => {
    ...
    if (step === 3) {
      ...
      if (selectedColors.length === 0) {
        isValid = false;
        const colorError =
          document.querySelector(".color-options").nextElementSibling;
